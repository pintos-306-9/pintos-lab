...
static void file_backed_destroy(struct page *page) {
    // page struct를 해제할 필요는 없습니다. (file_backed_destroy의 호출자가
    // 해야 함)
    struct file_page *file_page UNUSED = &page->file;
    if (pml4_is_dirty(thread_current()->pml4, page->va)) {
        file_write_at(file_page->file, page->va, file_page->read_bytes,
...
void *do_mmap(void *addr, size_t length, int writable, struct file *file,
    ...
    void *start_addr =
        addr; // 매핑 성공 시 파일이 매핑된 가상 주소 반환하는 데 사용
...
void do_munmap(void *addr) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        // 		// 매핑된 프레임이 있다면 = swap out 되지 않았다면 ->
        // 페이지를 제거하고 연결된 프레임도 제거 		spt_remove_page(spt, p); 	else
