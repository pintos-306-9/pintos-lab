...
bool anon_initializer(struct page *page, enum vm_type type, void *kva) {
    ...
    anon_page->slot_no = -1; // 초기화 함수가 호출되는 시점은 page가 매핑된
...
static bool anon_swap_in(struct page *page, void *kva) {
    ...
    for (e = list_begin(&swap_table); e != list_end(&swap_table);
        ...
        {
            for (int i = 0; i < 8; i++) {
                // 디스크, 읽을 섹터 번호, 담을 주소 (512bytes씩 읽는다. disk
                // 관련은 동기화 처리가 되어 있어서 lock 불필요)
                disk_read(swap_disk, page_slot_no * 8 + i,
                          kva + DISK_SECTOR_SIZE * i);
            }
            slot->page = NULL; // 빈 slot으로 업데이트한다.
            anon_page->slot_no =
                -1; // 이제 이 page는 swap_slot을 차지하지 않는다.
            lock_release(&swap_table_lock);
            return true;
        }
    }
...
static void anon_destroy(struct page *page) {
    ...
    // page struct를 명시적으로 해제할 필요는 없으며, 호출자가 이를 수행해야
    // 합니다.
    struct list_elem *e;
    struct slot *slot;
