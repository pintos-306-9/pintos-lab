...
bool lazy_load_segment(struct page *page, void *aux) {
    ...
    if (file_read(lazy_load_arg->file, page->frame->kva,
                  lazy_load_arg->read_bytes) !=
...
static bool setup_stack(struct intr_frame *if_) {
    ...
    // 스택은 아래로 성장하므로, USER_STACK에서 PGSIZE만큼 아래로 내린 지점에서
    // 페이지를 생성한다.
    ...
    // writable: argument_stack()에서 값을 넣어야 하니 True
    {
        ...
        if (success)
            // 3) rsp를 변경한다. (argument_stack에서 이 위치부터 인자를
            // push한다.)
            if_->rsp = USER_STACK;
    }
    return success;
